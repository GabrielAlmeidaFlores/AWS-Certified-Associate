...
AWS Data Encryption Keys (DEKs) are cryptographic keys used to encrypt and decrypt data stored in Amazon Web Services (AWS) environments. These keys are typically generated by AWS Key Management Service (KMS) and are used to protect data at rest, ensuring confidentiality and integrity. DEKs are symmetric keys, meaning the same key is used for both encryption and decryption, and they provide a high level of security when managing large-scale data encryption needs.

DEKs are typically wrapped by a master key, known as the Customer Master Key (CMK), which is stored and managed securely within AWS KMS. When data is encrypted, the DEK is used to perform the encryption, and the DEK itself is encrypted by the CMK and stored with the data. This layered encryption approach offers a balance between performance and security, as the DEKs can be rotated or regenerated periodically, while the CMK remains securely managed within AWS, reducing the risk of exposure.
